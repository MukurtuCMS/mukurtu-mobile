      ...
      if (atom.base_entity && atom.base_entity.fid) {
        ...
        fetch(state.siteUrl + '/app/file/' + fid + '.json', data)
          ...
          .then(async (file) => {
            ...
            // It appears that sometimes the payload is too large on these,
            // which we'll probably have to address. In the meantime catching
            // the error so it doesn't error out the app.
            try {
